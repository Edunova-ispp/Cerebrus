...
public void eliminarCurso(Long id) {
   ...
   @Override
public List<Integer> getNotaMediaPorActividad(Long cursoId, Long temaId) {
    Curso curso = cursoRepository.findByID(cursoId);
    if (curso == null) {
        throw new RuntimeException("404 Not Found");
    }
    ...
    List<ActividadAlumno> todasLasActividades = actividadAlumnoRepository.findByCursoID(cursoId);
    
    List<ActividadAlumno> actividadesDelTema = todasLasActividades.stream()
        .filter(aa -> aa.getActividad().getTema() != null &&
            aa.getActividad().getTema().getId().equals(temaId))
        .toList();

    List<Long> actividadesIds = new ArrayList<>();
    for (ActividadAlumno aa : actividadesDelTema) {
    ...
    for (Long actividadId : actividadesIds) {
        ...
        int notaMedia = contadorNotas > 0 ? (int) Math.round((double) sumaNotas / contadorNotas) : 0;
