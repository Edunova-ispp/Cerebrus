...
| :------: | :--------: | :----------- | :--------------------------- |
|   1.0   | 12/05/2026 | María Auxiliadora Quintana | Creación del documento base |
|          |            |              |                              |
...
**Usuarios Pilotos y Viabilidad**

Se debe consolidar un grupo de entre 10 y 12 usuarios piloto fuertemente comprometidos, abarcando los roles de alumno, docente y director. Para ello, se establecerá un plan estructurado con calendario de reuniones y registro riguroso de cada sesión, priorizando el contacto en vivo para captar matices de lenguaje corporal. Durante estas sesiones, se recogerá feedback explícito sobre la disposición al pago (preguntando precios abiertos sin rangos) y se monitorizará el uso real de la aplicación.

En caso de detectar un uso nulo o limitado, se investigarán las causas profundas para obtener insights que orienten el pivotaje. Si el acceso a las aulas se ve restringido, se documentará el uso en entornos alternativos, como el hogar o el entorno no escolar del profesor, explicitando siempre los motivos del bloqueo en centros y las acciones mitigadoras tomadas (ej. contacto directo con familias).

Todo el feedback recolectado será categorizado y priorizado para alimentar directamente el backlog de producto, transformando las sugerencias en mejoras iterativas. Finalmente, este proceso debe integrarse con un análisis de la viabilidad económica del nicho (centros privados vs. públicos y su capacidad presupuestaria) y con las acciones previstas de marketing para la preparación del lanzamiento oficial, habiendo implementado las acciones mitigadoras necesarias.
...
La presentación debe estructurarse en bloques temáticos grandes que respondan a preguntas clave, partiendo siempre de la idea principal para ir al detalle, evitando el hilo conductor caótico detectado en sesiones anteriores, evitando decir frases como "se explicará posteriormente". Se debe iniciar con un killer opener corto, potente y relacionado directamente con el proyecto, utilizando transiciones visuales sutiles (fade/dissolve) y evitando comenzar con frases explícitas para leer textualmente. Los ponentes deben proyectar seguridad, energía y alegría para generar enganche desde el primer momento. A nivel visual, hay que eliminar porcentajes y secciones de avance que sobrecarguen las diapositivas, reducir el texto y apostar por gráficos, metáforas visuales e imágenes. Cuando se incluya texto redactado, debe sincronizarse con el discurso oral leyéndolo textualmente para enfatizar el mensaje. Se debe distinguir entre la profundidad del análisis realizado (ej. costes) y el nivel de detalle mostrado en las diapositivas, el cual debe ser siempre asimilable en el tiempo previsto para cada sección. Se debe hablar más alto, controlar el tiempo y mejorar la fluidez en los cambios de ponente. Evitar frases del tipo "esto lo comentamos más tarde". Establecer un proceso formal de revisión con al menos tres revisores y ensayo (para que haya más fluidez) con tres días de antelación. Dándole importancia a la energía y al ritmo para que sean los adecuados. Arrancar el killer opener sin esperar a que haya silencio total. Es prioritario optimizar el tiempo de exposición eliminando conceptos técnicos estándar (como el flujo de Pull Requests) y filtrando el contenido para centrarse exclusivamente en los requerimientos actuales, omitiendo hitos o peticiones de fases ya superadas. Cuando se hable del team building, hablar de las tanto de las próximas quedadas como las ya realizadas. Cuando se hable del feedback de los usuarios pilotos, hablar del seguimiento actual (como lo estamos haciendo) además del futuro. Tener cuidado con la estética, porque puede dar la sensación que le damos más importancia a una cosa, cuando en verdad no. No poner demasiada información en las transparencias, solamente lo justo y necesario, lo demás simplemente nombrarlo; además de no poner transparencias con solo texto.

La presentación tiene que ser autocontenida, ya que va a haber gente que no conozca la aplicación. 

***Presentación 1***

No asumir demasiadas cosas, especialmente tener más cuidado en las demos. En la parte del equipo, poner una foto de todos y explicar por encima quienes somos. Antes de poner la demo, decir que problemas hemos tenido y como se han soluccionado.

***Presentación 2***

Está bien poner distitnas opiniones y perspectivas. Hablar más de lo que hemos hecho que del futuro (es más importante el pasado). Especificar como se está usando la aplicación en los colegios.

Hay que simplificar el acceso a la versión en producción mediante enlaces con lenguaje natural, eliminando tecnicismos que dificulten la entrada a perfiles directivos. Además, integrar de forma explícita la estrategia de escalabilidad a largo plazo, detallando cómo el producto captará nuevos segmentos de mercado y asegurará su viabilidad comercial frente a la competencia.

Tener para ambos escenarios, tanto para el pesimista como el optimista, tener datos concretos y números para que sea más visual. Ajustar el tamaño de la vista (concretamente de las gráficas) para que se pueda ver bien.

**Demo y vista de la aplicación**

La demo debe centrarse en primaria, en nuestros usuarios reales (usando nombres reales, para poder decir quien interáctua en cada momento), ser narrada, manteniendo la historia en todo momento y presentarse con zooms que permitan ver bien los detalles, siguiendo un orden lógico y mostrando únicamente, visualmente claro, los casos de uso core (la opción de generar con IA, la personalización de las actividades y las estadísticas a las que puede acceder el profesor), preferiblemente en formato vídeo y no capturas estáticas (teniendo en cuenta los cambios de escenario). Debe quedar claramente explicado cómo se transita entre el perfil de alumno y el de profesor. La interfaz del profesor requiere mayor usabilidad y no debe trasladar la lógica de gamificación del alumno, ya que esto reduce su practicidad. Los mockups deben dejar de ser superficiales y mostrar explícitamente los elementos diferenciadores que aportan valor real al producto. Se valora también incorporar actividades orientadas a introducir a los alumnos en el pensamiento computacional, la ética y el uso de la IA. El hilo conductor debe basarse en un escenario realista con datos que parezcan reales en todo momento, nunca datos de prueba. El vídeo debe descargarse localmente, no reproducirse desde YouTube. Reducir el ritmo o quitar contenido para que dé tiempo a asimilar cada paso. Sugerencia de sustituir el código de acceso a cursos por palabras con significado fácil de recordar (ej. "perrito"). No poner los cursos personalizados, para un alumno. 
...
**Inversores y anuncio de inversores**

Hay que justificar el mercado, proporcionando razones por las cuales deberían de invertir en nuestra aplicación, añadiendo a esto último algún método cuantitativo para convencer. Además definir de forma explícita y concreta en qué estarían invirtiendo (hacia donde va esa inversión), es decir, lo principal en lo que estarían invirtiendo. Añadir a cada modelo de inversión que porponemos a cambio, beneficios a final de año, condiciones una vez que han invertido, el retorno de inversión previsto, tiempo, etc.
No dar distitnas opciones, se propone una única opción.

El anuncio de inversores debe ser llamativo, y con vista a los inversores no a los usuarios, detayando porque es una buena idea de negocio. Debe equilibrar el ritmo del discurso, garantizando que la propuesta de rentabilidad sea clara y persuasiva.
...
* Se realizó una especificación más detallada de los casos de uso core y de las razones por las que los clientes elegirían Cerebrus frente a otras alternativas, destacando el cálculo de estadísticas y la creación automatizada de actividades mediante IA. Eliminando así la subjetibidad del análisis de competidores, centrando la explicación exclusivamente en propiedades, características operativas y datos objetivos.
...
* Se desarrolló un análisis de riesgos más exhaustivo, con clasificación de riesgos y planes de contingencia definidos para cada uno; y se analizaron los problemas encontrados durante el proyecto poniendo en marcha los planes de contingencia, definiendo métricas concretas, umbrales y periodos temporadles para alcanzarlos.
...
* Se ha transformado el anuncio de inversores para que no parezca un anuncio de usuarios, Ahora el foco es puramente cunatitativo y estratégico, explicando por qué es una buena idea de negocio y no solo cómo funciona la app.
* Se dió a los inversores una sola opción, presentandola como oferta de inversión única y concreta, con el destino de fondo y las condicones claramente definidas.
* Se incorporó métodos cuantitativos para justificaar el potencial del mercado, abandonando las descripciones subjetivas por datos.
* Se ha implementado en la demo un hilo conductor con datos reales, escenarios imaginarios pero verosímeles que facilitan la compresión de los casos de uso core (IA, personalización y estadísticas)
* En la demo se ha añadido iconos visuales para identificar intantáneamente qué rol (alumno o profesor) está interactuando.
* Se ha implemntado un gráfico de cuadrante con las isglas de todos los miembros correlacionancdo el trabajo realizado (utilizando puntos de historias) con el desempeño real.
* e ha realizado una intervención presencial en un centro educativo de Dos Hermanas para monitorizar el uso de CerebrUS en condiciones reales. Esta acción ha permitido identificar barreras de adopción críticas y palancas de valor, transformando el feedback directo en acciones de pivotaje para optimizar la tasa de retención.
* Se ha diseñado la primera presentación para que sea entendible por alguien ajeno a la asignatura, eliminando referencias internas y centrándose en la esencia potente del producto. 
